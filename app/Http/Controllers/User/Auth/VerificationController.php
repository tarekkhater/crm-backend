...
use App\Http\Requests\User\Auth\ResendRequest;
use App\Http\Requests\User\Auth\VerificationRequest;
...
class VerificationController extends Controller
{
    ...
    public function resend(ResendRequest $request){
        try{
                ...
                $activeuser = ActiveUser::where("user_id",$user->id)->where('type','2')->first();
                if($activeuser){
                  $user->Active()->update([
                    "code"=>Hash::make($code),
                    "status"=>"0",
            ...
            $this->setMessage(__('app.register.sent'));
            $this->setStatus(200);
        }catch(\Exception $e){
            $this->setStatus(422);
