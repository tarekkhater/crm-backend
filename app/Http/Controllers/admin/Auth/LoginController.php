...
class LoginController extends Controller
{
    ...
    */
    public function login(LoginRequest $request){
        ...
        // Cache::put('user_token_'.$user->id, $token, 60);
        // $user->token=$token;
        
        // Cookie::queue("jwt", $token, 60);
        
        $ttl = JWTAuth::factory()->getTTL() * 60; // getTTL يرجع بالدقائق، نحولها لثواني

    // خزّن التوكن في Cache بنفس مدة صلاحية JWT
    Cache::put('user_token_'.$user->id, $token, $ttl);

    $user->token = $token;
    Cookie::queue('jwt', $token, $ttl / 60);
    ...
    public function loginAs(Request $request){
        ...
        // Generate a new JWT token for the user to impersonate
