...
namespace App\Http\Controllers\admin\CRM\Api\Users\Agents\Conversion;
        ...
        if($request->type == 2){
           if(auth()->user()->type_id != 6){
            ...
            $this->setData($users);
                ...
                // $ids = UserManager::where('admin_id',auth()->user()->id)->where('type','1')->pluck('user_id');
                $users = Admin::with(['teamleader','broker'])->whereIn('id',$ids)->where('type_id',7)->paginate(15);
                $this->setData($users);
            } 
        }else{
...
public function Blocked(Request $request){
    ...
    public function getClints($id){
        ...
        $users = Admin::whereIn('id',$pluckId)->where('type_id',$admin->sub_type_id)->paginate(15);
        // $user->load(['users']);
    ...
    public function getcustomers($id){
        ...
        $users = User::whereIn('id',$pluckId)->with(['userInfo','Manager'])->paginate(15);
        $this->setData($users);
        $this->setMessage("success");
        return $this->sendApiResonse();
    }
