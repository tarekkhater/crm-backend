<!DOCTYPE html>
<html lang="ar" dir="rtl">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <meta http-equiv="Content-Language" content="en">
  <title>الوعي المالي؟</title>

  <style>
    .footertext {
      font-size: 12px;
    }

    a {
      text-decoration: none;
    }

    @media (min-width: 640px) {
      .footertext {
        font-size: 16px;
      }
      
     #text-inside{
         width:100%;
         text-align:right;
         display:block;
     }

      /* Default styles (optional) */
      #table-data {
        width: 50%;
        /* Default width */
        height: auto;
        /* Default height */
      }
     

      /* For screen widths between 768px and 1024px */
      @media (min-width: 768px) and (max-width: 1000px) {
        #table-data {
          width: 50%;
          /* Adjust width for tablets or medium-sized screens */
        }
        #text-inside{
         width:100%;
         text-align:right;
         display:block
        }
      }

      /* For screens with a max width of 768px (e.g., tablets or large mobile) */
      @media (max-width: 768px) {
        #table-data {
          width: 100%;
          /* Adjust width for smaller devices */
        }
         #text-inside{
         width:100%;
         text-align:right;
         display:block
        }
      }

      /* For screens with a max width of 480px (e.g., small mobile) */
      @media (max-width: 480px) {
        #table-data {
          height: 100px;
          /* Adjust height for very small screens */
        }
         #text-inside{
         width:100%;
         text-align:right;
         display:block
        }
      }
  </style>
</head>

<body style="margin : 0px">
  <div
    style="align-items: center; justify-content: center; flex-direction: column; margin-top: 1.25rem; font-family : Nunito, sans-serif "
    dir="rtl">
    <section style="max-width: 42rem; background-color: #fff;    justify-self: anchor-center;">
      <table width="100%" cellpadding="0" cellspacing="0" dir="rtl" style="">
        <tr>
          <td style="width: 100%;        text-align: right;">
            <a href="#">
              <img src="https://backend.bbstechnology.net/Emails/logo-light.png" alt="tailwindtaplogo"
                style="width:153px;justify-content: center;" />
            </a>
          </td>
        </tr>
      </table>
      <table width="100%" cellpadding="0" cellspacing="0" dir="rtl">
        <tr>
          <td style="padding: 10px;">
            <section>
              <div style="
text-align: right;
width: 90%;
margin-top: 15px;
background-color: #d1eae3;
    padding: 10px;
    border: 1px solid #d1eae3;
border-radius: 5px;
">
                <div style="color:rgb(0, 0, 0);font-size: 22px;font-weight: 400;line-height: 21.78px;">
                  كود نسيت كلمة المرور
                  </span>
                </div>
              </div>
              <main style="text-align : start; padding-right : 20px; padding-right : 20px;">
                <div class="my-8 grid grid-cols-1 gap-6" style="text-align:right">
                  <p style="color:#011610;    font-size: 22px;
    font-weight: 400;">
                    مرحبا <b
                      style="color:#037353;font-weight : 700;font-size: 18px;line-height: 21.78px;font-family: system-ui;">amr
                      gamal.</b>
                  </p>
                  <p style="margin-top : 10px;color:#011610;    font-size: 22px;
    font-weight: 500;" class="text-[#242052CC]">
                    لقد تلقينا طلبًا لإعادة تعيين كلمة المرور لحسابك في الوعي المالي. <br />يرجى استخدام الكود التالي
                    لإعادة
                    تعيين كلمة المرور:
              </main>
            </section>
          </td>
        </tr>
      </table>
      <table id="table-data" cellpadding="0" cellspacing="0" dir="rtl"
        style="justify-self: anchor-center;margin-top:10px;margin-bottom:10px;">
        <tr>
          <t style="text-align:center;">

            <p style="font-weight : 700;font-size: 37px;line-height: 21.78px;text-align:center;color:#037353;   
padding: 10px;
;
">
              <b>0</b>
            </p>
          </td>


        </tr>
      </table>
      <table width="100%" cellpadding="0" cellspacing="0" dir="rtl" style="justify-self: anchor-center;">
        <tr>
          <td style="font-family: system-ui;color:#011610;width:100%;text-align:right;">
            <b>ابدأ بثلاث خطوات سهلة:</b>
          </td>
        </tr>
      </table>

      <table width="100%" cellpadding="0" cellspacing="0" dir="ltr" style="justify-self: anchor-center;margin-top:5px;">
        <tr>
          <td style="align-content:center!important;color:#011610;text-align:right;
            font-size: 17px;direction:rtl">
            <p style="    font-size: 16px;
    font-weight: 700;
    line-height: 26px;"><b style="font-family: system-ui;color:#037353;">أدخل الرمز </b>انتقل إلى صفحة إعادة تعيين كلمة
              المرور.
              <br />
              <span style="font-family: system-ui;color:#037353;">trade.alwaaialmali.com</span>
            </p>

          </td>
          <td style="margin-right:4px;width: 13%;height: 8%;     text-align: center;">
            <img src="{{asset('Emails/lock-svg.png')}}"
              style="margin-right:4px;width: 55%;height: 80%;" alt="illustration-1" />
          </td>
        </tr>

        <tr>
          <td style="align-content:center!important;color:#011610;text-align:right;
            font-size: 17px;direction:rtl">
            <p style="    font-size: 16px;
    font-weight: 700;
    line-height: 26px;"><b style="font-family: system-ui;color:#037353;">أدخل الرمز </b>أدخل الرمز الموجود أعلاه.</p>

          </td>
          <td style="margin-right:4px;width: 13%;height: 8%;     text-align: center;">
            <img src="{{asset('Emails/codes-svg.png')}}"
              style="margin-right:4px;width: 55%;height: 80%;" alt="illustration-1" />
          </td>
        </tr>

        <tr>

          <td style="align-content:center!important;color:#011610;text-align:right; 
            font-size: 17px;direction:rtl">
            <p style="    font-size: 16px;
    font-weight: 700;
    line-height: 26px;"><b style="font-family: system-ui;color:#037353;">إنشاء كلمة مرور جديدة</b> اتبع التعليمات الي
              <br />
              تعين كلمة مرور جديدة.
            </p>
          </td>
          <td style="margin-top:4px;width: 13%;height: 8%; text-align: center;">
            <img src="{{asset('Emails/passwords-svg.png')}}"
              style="margin-right:4px;width: 55%;height: 80%;" alt="illustration-1" />
          </td>
        </tr>
      </table>

      <table width="100%" cellpadding="0" cellspacing="0" dir="rtl" style="margin-top:10px;">
        <tr>
          <td style="
        background-image:url('https://backend.bbstechnology.net/Emails/bg-footer.png'); 
  background-size: cover; background-position: center; background-blend-mode: overlay;border-radius:27px;">
            <table width="100%" cellpadding="0" cellspacing="0" dir="rtl" style="padding:80px 20px;">
              <tr>
                <td style="width:100%;text-align:right;display:flex" id="text-inside">
                  <div class="text-white" style="color:#fff;width:70%;">
                    <h2 style="font-size: 29px;
                    font-weight: 700;" class="text-[25px] font-semibold">شكرا لاختيارك الوعي المالي</h2>
                    <p class="text-[14px]" style="    font-size: 19px;
    font-weight: 500;
    line-height: 40px;    width: 89%;">
                      نحن نتطلع إلى دعمك في رحلتك التجارية ومساعدتك في تحقيق أهدافك المالية.
                    </p>
                  
                  </div>
                   <a href="https://alwaaialmali.com"
                    style="font-size: 18px;
                        height: max-content;
                        margin: auto;
                    font-weight: 500;background-color:#037353;color:#fff;padding:10px 48px;border:1px solid #037353;border-radius:5px;text-decoration: none;"
                    class="bg-white color-black hover:text-white hover:bg-[#0070E4] rounded-sm py-2 px-8">ابدأ
                    الآن</a>
                </td>
              </tr>
            
            </table>
          </td>
        </tr>
      </table>

      <table width="100%" cellpadding="0" cellspacing="0" dir="rtl" style="margin-top:20px;">
        <tr>
          <td style="text-align: center;" dir="rtl">
            <a href="https://www.facebook.com/awarenessprograms">
              <img src="{{asset('Emails/Facebook.png')}}" style="width: 5%" alt="illustration-1" />
            </a>
            <a href="https://www.instagram.com/lwy_lmly">
              <img src="{{asset('Emails/Instagram.png')}}" style="width: 5%" alt="illustration-1" />
            </a>
            <a href="#">
              <img src="{{asset('Emails/Twitter.png')}}" style="width: 5%" alt="illustration-1" />
            </a>
          </td>
        </tr>
      </table>

      <table width="100%" cellpadding="0" cellspacing="0" dir="rtl" style="margin-top:10px;">
        <tr>
          <td style="background-color:#037353;color:#fff;    padding: 36px 36px 0px;
          margin-top:4px; border-radius:1px;" dir="rtl">

            <div class="px-6 my-4" style="text-align : center;    font-size: 14px;
    font-weight: 400;
    line-height: 18px;">
              لقد تلقيت هذا البريد لأنك قمت بالتسجيل للانضمام إلى منصة الوعي المالي كمستخدم أو منشئ. وهذا يوضح أيضًا أنك
              توافق
              على شروط الاستخدام وسياسات الخصوصية الخاصة بنا. إذا كنت لم تعد ترغب في تلقي رسائل البريد الإلكتروني منا،
              فانقر فوق رابط إلغاء الاشتراك أدناه لإلغاء الاشتراك.
              <p class="lg-flex gap-4" style="font-size: 9px;
    font-weight: 400;
    line-height: 18px;justify-content: center;" dir="ltr">
                <span class="underline underline-offset-4"> +41 44 214 62 62 </span>
                <img src="https://backend.bbstechnology.net/Emails/Ellipse.png" alt="illustration-1" class="w-1"
                  style=" align-self: center; " />
                <span class="underline underline-offset-4" style="color:#fff;"> info@alwaaialmali.com </span>
                <img src="https://backend.bbstechnology.net/Emails/Ellipse.png" alt="illustration-1" class="w-1"
                  style=" align-self: center; " />
                <span class="underline underline-offset-4">HCQ6+MC5، مسقط 132، عُمان New Work Business Center - Oman
                  Avenues
                  Mall</span>
              </p>
            </div>
            <div class="px-2 my-8 text-[12px]" style="text-align : right;    font-size: 12px;
    font-weight: 400;
    line-height: 21px;">
              <span class="">@2025 جميع الحقوق محفوظه لمنصه الوعي المالي</span>
            </div>
          </td>
        </tr>
      </table>

    </section>
  </div>
</body>

</html>