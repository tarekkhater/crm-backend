...
">
 ...
 <table width="100%" id="table-data" cellpadding="0" cellspacing="0" dir="ltr"
        style="width:100%;justify-self: anchor-center;margin-top:10px;padding:40px;margin-bottom:10px;border-top: 1px solid {{ config('email-branding.colors.highlight_bg') }};;border-bottom: 1px solid {{ config('email-branding.colors.highlight_bg') }};">
        <tr>
          <td style="text-align:center;">
            <img src="{{asset('Emails/success.png')}}"
              style="margin-left:4px;width: 10%;height: 80%;" alt="illustration-1" />
            <p style="font-weight : 700;font-size: 28px;text-align:center;color:{{ config('email-branding.colors.accent') }};    margin: 0px;">
             Deposit details
            </p>
            <p style="font-weight : 700;font-size: 37px;text-align:center;color:{{ config('email-branding.colors.accent') }};    margin: 5px 0px;">
              {{$data['amount']}}$
            </p>
...
<li style="font-weight: 700; font-size: 22px; text-align: left; color: {{ config('email-branding.colors.accent') }}; padding: 0px 10px 10px;">
  <b>Current Balance:</b> {{$data['total']}}
    ...
    font-weight: 700;
    line-height: 26px;"><b style="font-family: system-ui;color:{{ config('email-branding.colors.accent') }};">
