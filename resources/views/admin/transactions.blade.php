...
{{--                <span class="breadcrumb-item active">{{ $title }}</span>--}}
        ...
        <div class="br-pagebody">
            ...
            <div class="br-section-wrapper">
                ...
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10" style="padding-bottom: 50px">Transactions
               </h6>
                    ...
                    <table id="datatable1" class="table display table-bordered responsive">
                        ...
                        @foreach ($items as $item)
                            <tr>
                                ...
                                <td>{{ $count++ }}</td>
                                <td><a href="{{ route('admin.users.show', $item->user ? $item->user->id : 1) }}">{{ $item->user ? $item->user->first_name . ' '.$item->user->last_name : 'not found'}}</a></td>
                                <td>{{ $item->type }}</td>
...
{{--                                    <a data-toggle="modal" data-target="#modaldemo1" href="" class="{{ $item->status ? 'btn-danger' : 'btn-success' }} btn " data-toggle="tooltip" data-placement="top">{{ $item->status ? 'Un Approve': 'Approve' }}</a>--}}
                                   @if(check_permission('delete-transaction'))
                                    <a href="{{ route('admin.transaction.destroy', $item->id) }}" onclick="return confirm(&quot;Click Ok to delete Transaction.&quot;)" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Delete"><em class="fa fa-trash"></em>
                                    </a>
                                       @endif
                                </td>
