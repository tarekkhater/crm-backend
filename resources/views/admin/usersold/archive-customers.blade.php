        ...
        <div class="br-pagebody" id="vue-content">
            ...
            <div class="br-section-wrapper">
                ...
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10" style="padding-bottom: 50px">
                    ...
                    @if (check_permission('update-archieve-customers'))

                    <button class="btn btn-success m-1" data-toggle="modal" data-target="#bulkAction"> Bulk Action </button>

@endif
                    @if(check_permission('assign-manager-archieve-customers'))
...
@if(check_permission('assign-retention-archieve-customers'))
                        ...
                        @endif
@if(check_permission('set-archieve-users-as-free'))
                        <div class="dropdown d-inline">
                            <button class="btn btn-success m-1" type="button" v-on:click="setUsersAsFree()">Set Users As Free</button>
                        </div>
                        @endif
                        @if(check_permission('set-archieve-users-as-active'))
                        <div class="dropdown d-inline">
                            <button class="btn btn-success m-1" type="button" v-on:click="setUsersAsActive()">Set Users As Active</button>
                        </div>
                        @endif
...
@endif
                    ...
                    @if (check_permission('create-archieve-customers'))
                        <a href="{{ route('admin.users.create-archieve-users') }}?r=user"><button class="btn btn-warning m-1" style="float: right"> Add  Customer</button></a>
...
@section('js')
    ...
    <script>
        ...
        let setUsersAsActiveUrl = "{{ route('admin.lead.set-users-as-active') }}";
            ...
            new Vue({
                ...
                methods: {
                    ...
                    setUsersAsActive () {
                        ...
                        this.changeManagerData.selected_users = this.selected_users // attach the selected users
                        this.changingManager = true;
                        axios.post(setUsersAsActiveUrl, this.changeManagerData).then((res) => {
                            this.changingManager = false;
                            window.location.reload();
                        }).catch((error) => {
                            this.changingManager = false;
